// ModernBERT model implementation using mlx-rs.
// Ported from candle_transformers::models::modernbert via recall.
...
pub(crate) struct ModernBert {
    ...
    /// Cached local attention mask keyed by seq_len. Avoids O(seq_len²) reallocation per forward().
...
fn get_local_attention_mask(seq_len: i32, half_window: i32) -> Result<Array, Exception> {
    ...
    for i in 0..n {
        for j in 0..n {
            if (j as isize - i as isize).unsigned_abs() > hw {
...
mod tests {
    ...
    // T-006: forward with 5 tokens → shape [1, 5, 768]
    #[test]
    #[serial]
    fn test_006_forward_5_tokens() {
    ...
    // T-007: forward with 3 tokens → shape [1, 3, 768]
    #[test]
    #[serial]
    fn test_007_forward_3_tokens() {
        let config = test_config();
        let mut model = ModernBert::new(&config).expect("create model");

        let input_ids: Vec<u32> = vec![1, 2, 3];
        let mask: Vec<u32> = vec![1, 1, 1];
        let output = model
            .forward(&input_ids, &mask, 1, 3)
            .expect("forward pass");
        assert_eq!(output.shape(), &[1, 3, 768]);
    }

    // T-008: prepare_4d_attention_mask with [1,1,0]
    #[test]
    #[serial]
    fn test_008_global_mask_values() {
    ...
    // T-009: all-ones mask → all zeros
    #[test]
    #[serial]
    fn test_009_global_mask_all_ones() {
    ...
    // T-010: local attention mask seq_len=5, half_window=1
    #[test]
    #[serial]
    fn test_010_local_mask_window() {
        ...
        for i in 0..5usize {
            for j in 0..5usize {
                ...
                if dist <= 1 {
                    assert_eq!(val, 0.0, "position ({i},{j}) within window should be 0.0");
                } else {
                    assert!(
                        val.is_infinite() && val.is_sign_negative(),
                        "position ({i},{j}) outside window should be -inf, got {val}"
                    );
                }
            }
        }
    ...
    // T-011: local attention mask seq_len=3, half_window=0
    #[test]
    #[serial]
    fn test_011_local_mask_zero_window() {
    ...
    // T-012: ModernBert::load nonexistent → SafeTensors load error
    #[test]
    #[serial]
    fn test_012_load_nonexistent_path_errors() {
        let config = test_config();
        let result = ModernBert::load("/nonexistent/model.safetensors", &config);
        let err = result.unwrap_err();
        assert!(
            err.to_string().contains("SafeTensors load error"),
    ...
    fn load_safetensors_correct_layers() {
        ...
        let model = ModernBert::load(&model_path, &config).expect("load model");
