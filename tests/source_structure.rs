...
// T-115: main.rs stays under 400 non-test lines after extraction
#[test]
fn main_rs_under_400_non_test_lines() {
    ...
    assert!(
        lines < 400,
        "[T-115] main.rs has {lines} non-test lines, expected < 400"
    );
}

// T-116: each command file stays under 200 non-test lines
#[test]
fn each_command_file_under_200_non_test_lines() {
    ...
    for filename in &expected_files {
        ...
        assert!(
            path.exists(),
            "[T-116] src/commands/{filename} does not exist"
        );
        let lines = count_non_test_lines(&path);
        assert!(
            lines < 200,
            "[T-116] commands/{filename} has {lines} non-test lines, expected < 200"
        );
    }
}
...
// T-119: embed_query warn! includes %query structured field
#[test]
fn embed_query_warn_includes_query_field() {
    let path = src_dir().join("commands").join("search.rs");
    assert!(
        path.exists(),
        "[T-119] src/commands/search.rs does not exist"
    );
    let source = fs::read_to_string(&path).unwrap();
    ...
    let mut found = false;
    for line in source.lines() {
        if (line.contains("embed_query") || line.contains("warn!")) && line.contains("%query") {
            found = true;
            break;
        }
    }
    ...
    if !found {
        ...
        assert!(
            count >= 1,
            "[T-119] run_search should contain at least 1 occurrence of '%query' \
             in a warn! call for embed_query failure, found {count}"
        );
    }
}

// T-120: each commands/*.rs has #[cfg(test)] module with at least 1 test
#[test]
fn each_command_file_has_test_module() {
    ...
    for filename in &expected_files {
        ...
        assert!(
            path.exists(),
            "[T-120] src/commands/{filename} does not exist"
        );
        let source = fs::read_to_string(&path).unwrap();
        assert!(
            source.contains("#[cfg(test)]"),
            "[T-120] commands/{filename} should contain #[cfg(test)] module"
        );
        assert!(
            source.contains("#[test]"),
            "[T-120] commands/{filename} should contain at least one #[test] function"
        );
    }
}

// T-121: vec_search warn! includes %query structured field
#[test]
fn vec_search_warn_includes_query_field() {
    ...
    assert!(
        count >= 1,
        "[T-121] hybrid_search should contain '%query' in vec_search warn!, found {count}"
    );
}

// T-122: vec_search warn! includes candidate_limit structured field
#[test]
fn vec_search_warn_includes_candidate_limit() {
    ...
    assert!(
        count >= 1,
        "[T-122] hybrid_search should contain 'candidate_limit' in vec_search warn!, found {count}"
    );
}
